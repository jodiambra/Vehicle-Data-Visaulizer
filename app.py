...
if looking :
    ...
    # vehicle cylinders
    vehicle_cylinder_fig = px.bar(vehicle_cylinder, x='cylinder', y='count', title='Vehicle Cylinders')
    ...
    vehicle_condition_fig = px.bar(vehicle_condition, x='condition', y='count', title='Vehicle Condition')
    ...
    vehicle_milage_high = vehicles[['model','price', 'odometer']].nlargest(20, columns='odometer').reset_index(drop=True)
    
    # highest milage vehicles
    st.header('Half a Million Miles Cars...')
    vehicle_milage_high_fig = px.bar(vehicle_milage_high, x='model', y=(['odometer', 'price']), title='Vehicle Milage', log_y=True)
    ...
    vehicle_model_year_cost = vehicles.groupby('model_year')['price'].mean().reset_index()
    vehicle_model_year_cost = pd.DataFrame(vehicle_model_year_cost)
    
    st.plotly_chart(px.histogram(vehicle_model_year_cost, x='model_year', y='price'))

    #-------------------------------------------#

    # correlation of price and other variables

    st.header('What is th Greates Contributor to Vahicle Price')
